...
OPTIONS.lfstk_key_dir = None
OPTIONS.lfstk_path = None
...
def LoadRecoveryFSTab(zip):
  ...
  for line in data.split("\n"):
    ...
    if options:
      ...
      for i in options:
        if i.startswith("size_hint="):
          p.size_hint = int(i[10:])
...
def StitchImage(source_filename):
  if not os.access(source_filename, os.F_OK):
    return None
  img = tempfile.NamedTemporaryFile()
  if not OPTIONS.lfstk_key_dir:
    assert False, "--lfstk_keydir not specified!"
  ret = lfstk_wrapper.stitch_images([source_filename],
          OPTIONS.info_dict["mfld_signed_osupdates"] == "true",
          OPTIONS.info_dict["mfld_board_stepping"], OPTIONS.lfstk_key_dir,
          img.name, OPTIONS.lfstk_path)
  assert ret == 0, "LFSTK returned an error"
  p = Run(["osip_reader", "-v", img.name])
  p.wait()
  assert p.returncode == 0, "OS image verification failed"
...
def GetStitchedImage(name, unpack_dir, filename):
  img = StitchImage(os.path.join(unpack_dir, filename))
  if not img:
    print "Data for %s not found, skipping" % (filename,)
    return None
...
def BuildBootableImage(sourcedir):
  ...
  if (not os.access(os.path.join(sourcedir, "RAMDISK"), os.F_OK) or
      not os.access(os.path.join(sourcedir, "bootstub"), os.F_OK) or
      not os.access(os.path.join(sourcedir, "cmdline"), os.F_OK) or
      not os.access(os.path.join(sourcedir, "kernel"), os.F_OK)):
    return None

  ramdisk_img = tempfile.NamedTemporaryFile()
  os_img = tempfile.NamedTemporaryFile(suffix=".bin")
  img = tempfile.NamedTemporaryFile()
  ...
  mfld_osimage.make_osimage(os.path.join(sourcedir, "bootstub"),
          os.path.join(sourcedir, "kernel"),
          os.path.join(sourcedir, "cmdline"), ramdisk_img.name, os_img.name)
  ramdisk_img.close()

  ret = StitchImage(os_img.name)
  os_img.close()
  return ret
...
def GetBootableImage(name, prebuilt_name, unpack_dir, tree_subdir):
  ...
  if ret:
    ...
    img = BuildBootableImage(os.path.join(unpack_dir, tree_subdir))
    if not img:
      print "Image data for %s not found, skipping" % (tree_subdir,)
      return None
    return File(name, img)
...
COMMON_DOCSTRING = """
  ...
  -K  (--lfstk_keydir)  <path>
      Path to directory containing LFSTK signing keys.

  -P  (--lfstk_path)  <path>
      Path to LFSTK binary. If omitted, will search in $PATH for it.
"""
...
def ParseOptions(argv,
  ...
  try:
    opts, args = getopt.getopt(
        argv, "hvp:s:x:K:P:" + extra_opts,
        ["help", "verbose", "path=", "device_specific=", "extra=",
         "lfstk_keydir=", "lfstk_path="] + list(extra_long_opts))
  ...
  for o, a in opts:
    ...
    elif o in ("-K", "--lfstk_keydir"):
      OPTIONS.lfstk_key_dir = a
    elif o in ("-P", "--lfstk_path"):
      OPTIONS.lfstk_path = a
    else:
